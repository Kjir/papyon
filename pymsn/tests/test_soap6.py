...
class ABTests:
    ...
    def sso_cb2(self, soap_response, *tokens):
        ...
        for token in tokens:
            if token.service_address == SSO.LiveService.CONTACTS[0]:
                ...
                break
        self.abook.ABFindAll(self.contacts_cb)
        self.sharing.FindMembership(self.membership_cb)
    ...
    def contacts_cb(self, soap_response, contacts):
        for contact in contacts:
            ...
            print contact.id
        self.abook.ABGroupAdd("pymsn2XXXX", self.group_add_cb)

    def group_add_cb(self, soap_response, guid):
        print "The guid for the added group is " + guid
